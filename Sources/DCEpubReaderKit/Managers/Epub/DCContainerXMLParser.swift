...
final class DCContainerXMLParser: NSObject, XMLParserDelegate {
    ...
    func parse(url: URL) throws -> String {
        ...
        guard FileManager.default.fileExists(atPath: containerURL.path) else {
            throw DCEpubError.missingContainer
        }

        guard let parser = XMLParser(contentsOf: containerURL) else {
            throw DCEpubError.parseError("Unable to initialize XMLParser for container.xml")
        }
        ...
        guard parser.parse() else {
            throw DCEpubError.parseError("Invalid container.xml")
        }
