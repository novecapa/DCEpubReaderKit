...
final class DCNavXHTMLParser: NSObject, XMLParserDelegate {
    ...
    func parse(navURL: URL) throws -> [DCTocNode] {
        guard let parser = XMLParser(contentsOf: navURL) else {
            throw DCEpubError.parseError("Unable to initialize XMLParser for nav.xhtml")
        }
        ...
        guard parser.parse() else {
            throw DCEpubError.parseError("nav.xhtml is not a valid XML document")
        }
        ...
        if result.isEmpty {
            ...
            throw DCEpubError.parseError("No TOC <nav> found in nav.xhtml")
        }
        return result
    }
    ...
    private var stack: [DCTocNode] = []
    private var roots: [DCTocNode] = []
    ...
    func parser(_ parser: XMLParser,
        ...
        if lower == "li" {
            stack.append(DCTocNode(label: "", href: nil, children: []))
        ...
        if lower == "a" {
            ...
            if insideAnchorDepth == 0 {
                ...
                if !stack.isEmpty {
                    ...
                    last = DCTocNode(label: label, href: href, children: last.children)
                    stack.append(last)
                }
        ...
        if lower == "li" {
            ...
            if finished.label.isEmpty {
                ...
                if !fallbackLabel.isEmpty {
                    let patched = DCTocNode(label: fallbackLabel, href: finished.href, children: finished.children)
                    attach(node: patched)
                } else {
    ...
    private func attach(node: DCTocNode) {
