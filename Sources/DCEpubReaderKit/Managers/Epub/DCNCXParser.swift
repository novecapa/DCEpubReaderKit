...
final class DCNCXParser: NSObject, XMLParserDelegate {
    ...
    func parse(ncxURL: URL) throws -> [DCTocNode] {
        guard let parser = XMLParser(contentsOf: ncxURL) else {
            throw DCEpubError.parseError("Unable to initialize XMLParser for NCX")
        }
        ...
        guard parser.parse() else {
            throw DCEpubError.parseError("NCX is not a valid XML document")
        }
    ...
    private(set) var roots: [DCTocNode] = []
    ...
    func parser(_ parser: XMLParser,
        ...
        case Constants.content:
            ...
            if let src = attributes["src"], var last = stack.popLast() {
                last = DCTocNode(label: last.label, href: src, children: last.children)
                stack.append(last)
            }
    ...
    func parser(_ parser: XMLParser,
        ...
        case Constants.text:
            ...
            if collectingLabelText, let last = stack.popLast() {
                ...
                let patched = DCTocNode(label: label, href: last.href, children: last.children)
